...
func FullProperty(key string, defaultValue string, isSecret bool)string {
	...
	if k8sFound {
		...
		if isSecret {
			secret,err := clientset.CoreV1().Secrets("golodex").Get(context.TODO(), "golodex-secrets", metav1.GetOptions{})
			if err != nil {
				panic(err.Error())
			}
			if err != nil {
			...
			config,err := clientset.CoreV1().ConfigMaps("golodex").Get(context.TODO(), "golodex-api", metav1.GetOptions{})
			if err != nil {
				panic(err.Error())
			}
			if err != nil {
